...
//	test_a = 10; <- 위와 같이 함수의 지역 안에서 선언된 변수가 리턴되면서 사라졌는데 그 레퍼런스를 가리키고 있으므로 위험하게 된다.
	...
	*/
	...
	int c = 15;
	another_b = c;		/* <- 이렇게 보면 레퍼런스는 새로운 타깃으로 변경이 불가능한데 c를 대입하는 것으로 봐서 타깃이 변경되는 것처럼 보이는데
							이는 컴파일러가 c를 상수로서 처리를 해주기 때문에 에러가 나지 않는다.
							즉 another_b = 15로 변환이 되는 거라고 생각하면 이해하기 쉽다.
							이 증거로 만약 주소값이 c로 변경된거라면 아래에 c = 40 을 통해 another_b의 결과값이 40이 나와야 되는데 
							결과가 15로 나오는것을 보면 c의 주소값이 대입되지 않은 것을 알 수 있다.
						*/
	c = 40;
